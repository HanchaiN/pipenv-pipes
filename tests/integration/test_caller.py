...
from pipenv_pipes.caller import (
    call_venv,
    # call_python_version
    # call_shell,
)
...
def test_call_venv_not_a_venv(temp_folder):
    """ call venv on orphan folder: No virtual path has been created """
    output, code = call_venv(temp_folder, VenvType.PIPENV)
    assert code != 0
    assert 'no virtualenv has been created' in output.lower()


def test_call_venv(mock_env_home):
    ...
    output, code = call_venv(project_dir=project_dir, venv_type=VenvType.PIPENV)
...
@pytest.mark.skip
def test_call_shell(mock_env_home):
    ...
    # import pdb; pdb.set_trace()
    # output = call_shell(cwd=project_dir, timeout=5)
